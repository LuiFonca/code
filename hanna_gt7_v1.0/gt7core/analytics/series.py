...
def best_combined_sectors(
    ...
    total_ms = 0
    any_valid = False
    for ta, tb in zip(sector_times_a, sector_times_b, strict=False):
        if ta is None and tb is None:
            choices.append(None)
            continue
        # Reordenado para que o narrowing seja provável: na forma anterior o
        # mypy não conseguia deduzir que `ta` não era None dentro do ramo A.
        # Comportamento idêntico — o caso (None, None) já saiu no `continue`.
